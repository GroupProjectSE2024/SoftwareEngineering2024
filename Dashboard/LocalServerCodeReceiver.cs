...
{
    private readonly string _redirectUri; // Private readonly field to store the redirect URI
    ...
    public LocalServerCodeReceiver(string redirectUri) // Constructor to initialize the redirect URI
    {
        ...
        this._redirectUri = redirectUri; // Assign the redirect URI to the private field
    }

    /// <summary>
    /// Gets the redirect URI.
    /// </summary>
    public string RedirectUri => _redirectUri; // Public property to get the redirect URI
    ...
    public async Task<AuthorizationCodeResponseUrl> ReceiveCodeAsync(AuthorizationCodeRequestUrl url, CancellationToken taskCancellationToken) // Asynchronous method to receive the authorization code
    {
        ...
        try
        {
            ...
            HttpListenerContext context = await listener.GetContextAsync().ConfigureAwait(false); // Wait for an incoming request

            // Extract query parameters before sending response
            NameValueCollection queryString = context.Request.QueryString; // Get the query string from the request
            ...
            using (HttpListenerResponse response = context.Response) // Get the response object
            {
